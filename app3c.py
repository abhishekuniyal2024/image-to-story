...
def main():
    ...
    with col2:
        ...
        if st.session_state.results is not None:
            ...
            # AUDIO OUTPUT SECTION (optional; only runs when user clicks a button)
            ...
            if st.session_state.final_text:
                ...
                with col_audio2:
                    if st.button("💾 Generate and download Audio", key="save_audio_btn"):
                        try:
                ...
                # Inline playback control within Audio Settings (no download UI)
                if st.session_state.last_audio_file and os.path.exists(st.session_state.last_audio_file):
                    st.markdown("---")
                    st.markdown("**Playback**")
                    if st.button("▶️ Play Generated Audio", key="play_full_audio_btn"):
                        try:
            ...
            # No separate download section; audio remains saved to the output folder.
